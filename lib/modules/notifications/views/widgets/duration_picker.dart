// ! Copied over from DurationPicker
...
const double _kDurationPickerWidthLandscape = 512;

const double _kDurationPickerHeightPortrait = 380;
const double _kDurationPickerHeightLandscape = 304;
...
class DialPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
      centerPoint,
      radius * 0.88,
      Paint()..color = context.theme.canvasColor,
    );
    ...
    canvas.drawCircle(handlePoint, 20, handlePaint);
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  double _getThetaForDuration(Duration duration, BaseUnit baseUnit) {
    final baseUnits = _getDurationInBaseUnits(duration, baseUnit);
    final baseToSecondaryFactor = _getBaseUnitToSecondaryUnitFactor(baseUnit);
  ...
  Duration _baseUnitToDuration(double baseUnitValue) {
    final unitFactor = _getBaseUnitToSecondaryUnitFactor(widget.baseUnit);
  ...
  List<TextPainter> _buildBaseUnitLabels(TextTheme textTheme) {
    ...
    switch (widget.baseUnit) {
      case BaseUnit.millisecond:
        const interval = 100;
        const factor = Duration.millisecondsPerSecond;
        const length = factor ~/ interval;
      ...
      case BaseUnit.second:
        const interval = 5;
        const factor = Duration.secondsPerMinute;
        const length = factor ~/ interval;
        baseUnitMarkerValues = List.generate(
          length,
      ...
      case BaseUnit.minute:
        const interval = 5;
        const factor = Duration.minutesPerHour;
        const length = factor ~/ interval;
        baseUnitMarkerValues = List.generate(
          length,
      ...
      case BaseUnit.hour:
        const interval = 3;
        const factor = Duration.hoursPerDay;
        const length = factor ~/ interval;
